...
#         # change here to 47
#         self.fc3 = nn.Linear(in_features=64, out_features=47)
...
def predicted_values(model, data_loader):
    ...
    for i, (x, y) in enumerate(data_loader):
        ...
        out = model(x)
        # change [1] to [0] to get the prediction "strength"
        pred = out.data.max(1, keepdim=False)[0]
        pred = torch.exp(pred)  # convert to probabilities
